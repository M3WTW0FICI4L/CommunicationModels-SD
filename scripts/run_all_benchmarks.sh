...
if [ "$SKIP_DIRECT" != "true" ]; then
    ...
    if docker compose -f docker/docker-compose.direct.yml up -d --build 2>&1 | head -20; then
        sleep 5
        ...
        docker compose -f docker/docker-compose.direct.yml down || true
        sleep 2
    else
...
if [ "$SKIP_INDIRECT" != "true" ]; then
    ...
    if docker compose -f docker/docker-compose.indirect.yml up -d --build --scale worker=4 2>&1 | head -20; then
        ...
        docker compose -f docker/docker-compose.indirect.yml down || true
        sleep 2
    else
...
ls -lh results/direct/*.json 2>/dev/null | awk '{print "  " $9, "(" $5 ")"}' || echo "  (No results found)"

echo ""
echo "Indirect Architecture Results:"
ls -lh results/indirect/*.json 2>/dev/null | awk '{print "  " $9, "(" $5 ")"}' || echo "  (No results found)"

echo ""
log_success "Benchmark orchestration complete!"
echo ""
echo "Next steps:"
echo "  1. View results: ls -lh results/direct/ results/indirect/"
echo "  2. Plot results: python3 scripts/plot_results.py"
echo "  3. Analyze JSON: jq '.summary' results/direct/*.json"
echo ""
